...
|---|---|---|
| `DATA_DIR` | `"DATA"` | Root directory containing one sub-folder per brand |
| `IMG_SIZE` | `(224, 224)` | All images are resized to this resolution |
| `BATCH_SIZE` | `64` | Number of images per training batch |
| `EPOCHS` | `1` | Total epochs across both training phases |
| `BASE_LR` | `0.001` | Learning rate for Phase 1; Phase 2 uses `BASE_LR / 10` |
...
**`Rescaling(1/255)`**
Raw pixel values range from 0–255. Dividing by 255 maps them to [0, 1]. Neural networks train much more stably on small, normalised values — large inputs cause large activations which make gradients harder to control.

---

**Augmentation layers (`RandomFlip`, `RandomRotation`, `RandomZoom`, `RandomContrast`)**
These layers randomly perturb each image during training only — during validation and inference they are automatically bypassed. The goal is to make the model invariant to minor visual differences: a car flipped horizontally, slightly rotated, or photographed in different lighting should still be recognised as the same brand. This reduces overfitting without needing extra data on disk.
...
| Layer | Role |
|---|---|
| `Conv2D(32, 3×3)` | First convolution. Learns 32 simple patterns: edges, corners, colour gradients. The 3×3 kernel looks at a small neighbourhood of pixels at a time. |
| `BatchNormalization` | Normalises the output of the conv layer across the batch. Stabilises training, allows higher learning rates, and slightly regularises the model. |
| `Conv2D(32, 3×3)` | Second pass over the feature maps — refines the 32 patterns detected by the first conv. Two convolutions in sequence effectively give a larger receptive field than one. |
| `BatchNormalization` | Same stabilisation as above. |
| `MaxPooling2D` | Takes the maximum value in each 2×2 region, halving spatial dimensions (e.g. 224→112). Reduces computation and makes detections slightly position-invariant. |
| `Dropout(0.1)` | Randomly zeroes 10% of neurons during each training step. Forces the network to not rely on any single neuron, acting as a light regulariser. |

---

**Block 2 — 64 filters (mid-level features + channel mixing)**

| Layer | Role |
|---|---|
| `Conv2D(64, 3×3)` | Doubles the filter count to 64. Learns more complex combinations of the Block 1 features: textures, simple shapes like grilles, headlights. |
| `BatchNormalization` | Stabilisation. |
| `Conv2D(64, 3×3)` | Further refines the 64 feature maps. |
| `BatchNormalization` | Stabilisation. |
| `Conv2D(64, 1×1)` | A **1×1 convolution** does not look at spatial neighbours — it mixes information *across channels*. Think of it as a learned weighted sum of the 64 feature maps at each pixel. Adds non-linear capacity cheaply and acts as a "squeeze" that helps the network decide which features matter. |
| `BatchNormalization` | Stabilisation. |
| `MaxPooling2D` | Spatial downsampling (112→56). |

---

**Block 3 — 128 filters (wider receptive field)**

| Layer | Role |
|---|---|
| `Conv2D(128, 5×5)` | A **5×5 kernel** scans a wider area than 3×3, capturing larger structures like bumper shapes, roof lines, or windshield proportions in a single step. Useful for car recognition where global shape matters. |
| `BatchNormalization` | Stabilisation. |
| `Conv2D(128, 3×3)` | Follows up with a 3×3 to sharpen and refine the wider features. |
| `BatchNormalization` | Stabilisation. |
| `MaxPooling2D` | Spatial downsampling (56→28). |
| `SpatialDropout2D(0.2)` | Unlike regular Dropout which zeroes individual values, `SpatialDropout2D` drops **entire feature maps** (channels). This is more effective for CNNs because adjacent pixels in a feature map are highly correlated — dropping one pixel doesn't help much, but dropping the whole channel forces other channels to carry the information. |
...
| Layer | Role |
|---|---|
| `Conv2D(256, 3×3) × 3` | Three consecutive convolutions with 256 filters. At this depth, the network detects high-level, abstract features: brand-specific styling elements, badge shapes, characteristic curves. Three layers in sequence provide a very deep effective receptive field. |
| `BatchNormalization × 3` | After each conv. |
| `GlobalAveragePooling2D` | Instead of flattening the entire 28×28×256 tensor (=200,704 values) into a vector, GAP **averages each of the 256 feature maps** into a single number, producing a 256-d vector. This dramatically reduces parameters going into the head, is more spatially invariant, and is much less prone to overfitting than `Flatten`. |
...
| Layer | Role |
|---|---|
| `Dense(512, relu, name="features")` | A fully-connected layer that combines the 256 GAP features into 512 higher-level representations. Named `"features"` because this 512-d vector can be extracted as an image embedding for similarity search or a lookup database. |
| `Dropout(0.5)` | Aggressive 50% dropout before the final layer — the strongest regularisation point in the network. Prevents the dense layer from memorising training examples. |
| `Dense(num_classes, softmax)` | Output layer. Produces a probability distribution over all brands. The class with the highest probability is the prediction. |
